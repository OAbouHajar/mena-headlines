 ...
 * - Generates one new report per day at 12:00 GMT
 * - Keeps last 7 daily reports per language
...
const MAX_HISTORY    = 7;   // keep 7 daily reports

// Returns the UTC timestamp of the most recent 12:00 noon boundary
function getDailyNoonBoundary() {
  const now = new Date();
  const todayNoon = Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), 12, 0, 0);
  return now.getTime() >= todayNoon ? todayNoon : todayNoon - 24 * 60 * 60 * 1000;
}
...
module.exports = async function (context, req) {
  ...
  try {
    ...
    if (container) {
      const index = await readIndex(container, requestLang);
      if (index.length > 0 && index[0].generatedAt >= getDailyNoonBoundary()) {
    ...
    if (container) {
      ...
      // AI chat posting disabled — to re-enable, restore postAiChatMessage() from api/chat-timer/index.js
      // and call it here: if (requestLang === 'ar') { postAiChatMessage(headlines).catch(() => {}); }
    }
